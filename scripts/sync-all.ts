...
function formatGatewaySyncMessage(gateway: GatewayId, error: unknown) {
  const message =
    error instanceof Error ? error.message : `Unknown ${gateway} sync error`;
...
function filterExcluded(payments: StoredPayment[]) {
  return payments.filter(
    (payment) => !isExcludedWebinarWeek(payment.webinarDate),
  );
}
...
function isDeterministicMirrorDuplicate(
  candidate: StoredPayment,
  existing: StoredPayment,
) {
  if (
    candidate.sourceGateway !== "cashfree" ||
    existing.sourceGateway !== "instamojo"
  ) {
    return false;
  }

  const { identityMatch, amountMatch, timeDiffMs } = scoreDuplicateCandidate(
    candidate,
    existing,
  );
  return (
    candidate.purpose.startsWith("MOJ") &&
    identityMatch &&
    amountMatch &&
...
function isGenericCrossGatewayDuplicate(
  candidate: StoredPayment,
  existing: StoredPayment,
) {
  ...
  const { identityMatch, amountMatch, classificationMatch, timeDiffMs } =
    scoreDuplicateCandidate(candidate, existing);
  ...
  for (const payment of sorted) {
    ...
    if (match) {
      ...
      if (
        ...
        if (
          payment.sourceGateway === "instamojo" &&
          match.sourceGateway === "cashfree"
        ) {
          const index = unique.findIndex(
            (entry) => entry.paymentId === match.paymentId,
          );
          if (index >= 0) {
...
function mergeStoredPayments(
  primary: StoredPayment,
  secondary: StoredPayment,
): StoredPayment {
  return {
    ...secondary,
    ...primary,
...
async function fetchAllInstamojoPages<T>(
  endpoint: string,
  key: string,
): Promise<T[]> {
  const items: T[] = [];
...
function normalizeInstamojoRequests(
  paymentRequests: InstamojoPaymentRequest[],
) {
...
async function syncInstamojo(generatedAt: string): Promise<GatewaySyncResult> {
  ...
  const payments = await fetchAllInstamojoPages<InstamojoPayment>(
    "payments",
    "payments",
  );
  const normalizedRequests = normalizeInstamojoRequests(paymentRequests);
  const normalizedPayments = filterExcluded(
    normalizePayments(paymentRequests, payments),
  );
  const dashboard = {
...
async function fetchPayuPaymentLinksByOauth(
  generatedAt: string,
): Promise<GatewaySyncResult> {
  const merchantId = optionalEnv("PAYU_MERCHANT_ID");
  if (!merchantId) {
    throw new Error(
      "PayU merchant ID is required for OAuth payment-link sync.",
    );
  }
  ...
  for (const window of dayRangeWindows(HISTORY_START, today, 31)) {
    ...
    while (keepGoing) {
      ...
      if (!response.ok) {
        const message = await response.text();
        throw new Error(
          `PayU payment-links request failed: ${response.status} ${message}`,
        );
      }

      const payload = (await response.json()) as {
        result?: {
          paymentLinksList?: Array<{
            invoiceNumber: string;
            description?: string | null;
            createDate?: string;
            amount?: number;
  ...
  for (const link of links) {
    ...
    while (keepGoing) {
      const url = new URL(
        `${PAYU_PAYMENT_LINKS_BASE}/${link.invoiceNumber}/txns`,
      );
      url.searchParams.set("pageSize", "100");
      url.searchParams.set("pageOffset", String(pageOffset));
      ...
      if (!response.ok) {
        const message = await response.text();
        throw new Error(
          `PayU transaction-details request failed: ${response.status} ${message}`,
        );
      }
  ...
  return {
    dashboard: buildDashboardDataFromStoredPayments(
      ...
      filteredPayments,
      {
        ...
        successfulPaymentCount: filteredPayments.filter(
...
async function fetchPayuByLegacyBackfill(
  ...
  if (!key || !salt) {
    throw new Error(
      "PayU key and salt are not configured for legacy transaction sync.",
    );
  }

  const mergedPayments = new Map(
    existingPayments.map((payment) => [payment.paymentId, payment]),
  );
  const today = new Date();
  const historyStartDate = new Date(`${HISTORY_START}T00:00:00+05:30`);
  const completedBackfill =
    nextWindowEnd === null && existingPayments.length > 0;
  let activeWindowEnd = nextWindowEnd
    ? new Date(`${nextWindowEnd}T00:00:00+05:30`)
    : today;
  ...
  while (activeWindowEnd >= historyStartDate) {
    let activeWindowStart = addDays(
      activeWindowEnd,
      -(PAYU_LEGACY_WINDOW_DAYS - 1),
    );
    activeWindowStart = clampToHistoryStart(activeWindowStart);

    if (completedBackfill) {
      const recentStart = clampToHistoryStart(
        addDays(today, -(PAYU_LEGACY_RECENT_REFRESH_DAYS - 1)),
      );
    ...
    if (!response.ok) {
      const message = await response.text();
      throw new Error(
        `PayU legacy request failed: ${response.status} ${message}`,
      );
    }
    ...
    if (payload.status === 0) {
      ...
      throw new Error(
        `PayU legacy transaction API error: ${payload.msg ?? "Unknown error"}`,
      );
    }
    ...
    for (const details of parsePayuLegacyTransactions(payload)) {
      const paymentId = String(
        details.id ?? details.mihpayid ?? details.txnid ?? "",
      );
      if (!paymentId) continue;

      const amount = Number(details.amount ?? details.transaction_fee ?? 0);
      const purpose = String(
        details.productinfo ?? details.field9 ?? "PayU Payment",
      );
  ...
  return {
    ...
    payments: filteredPayments,
    backfillState: {
      nextWindowEnd: completed
        ? null
        : activeWindowEnd.toISOString().slice(0, 10),
      completed,
      lastFetchedStart,
      lastFetchedEnd,
    },
...
async function syncPayu(
  ...
  const existingPayments = firestore
    ? await readStoredPayments(firestore, "payuPayments").catch(() => [])
    : [];
  const progress = firestore
    ? await readBackfillState(firestore, "payuBackfill").catch(() => null)
    : null;
  const nextWindowEnd =
  ...
  try {
    ...
    const legacyResult = await fetchPayuByLegacyBackfill(
      generatedAt,
      existingPayments,
      nextWindowEnd,
    );
    ...
    for (const payment of oauthResult.payments) {
      ...
      mergedPayments.set(
        payment.paymentId,
        existing ? mergeStoredPayments(payment, existing) : payment,
      );
    }
    ...
    const dashboard = buildDashboardDataFromStoredPayments(
      ...
      merged,
      {
        ...
        successfulPaymentCount: merged.filter(
          (payment) => payment.status === "Credit",
        ).length,
      },
      generatedAt,
  ...
  return fetchPayuByLegacyBackfill(
    generatedAt,
    existingPayments,
    nextWindowEnd,
  );
...
async function syncCashfree(generatedAt: string): Promise<GatewaySyncResult> {
  ...
  const reconUrl =
    optionalEnv("CASHFREE_RECON_URL") ?? DEFAULT_CASHFREE_RECON_URL;
  const payments: StoredPayment[] = [];
  const cashfreeHistoryStart = new Date();
  cashfreeHistoryStart.setDate(
    cashfreeHistoryStart.getDate() - CASHFREE_MAX_LOOKBACK_DAYS,
  );
  const effectiveHistoryStart = [
    ...
    .sort()
    .at(-1) as string;
  const windows = dayRangeWindows(
    effectiveHistoryStart,
    new Date(),
    CASHFREE_WINDOW_DAYS,
  );
  ...
  for (const window of windows) {
    ...
    do {
      ...
      if (!response.ok) {
        const message = await response.text();
        throw new Error(
          `Cashfree reconciliation request failed: ${response.status} ${message}`,
        );
      }
      ...
      for (const item of payload.data ?? []) {
        const orderDetails =
          (item.order_details as Record<string, unknown> | undefined) ?? {};
        const customerDetails =
          (item.customer_details as Record<string, unknown> | undefined) ?? {};
        const paymentDetails =
          (item.payment_details as Record<string, unknown> | undefined) ?? {};
        const eventDetails =
          (item.event_details as Record<string, unknown> | undefined) ?? {};
        const tags =
          (orderDetails.order_tags as
...
async function writeGatewayToFirestore(
  ...
  if (
    result.dashboard.syncStatus.state === "error" &&
  ...
  await firestore
    .collection("dashboardMetadata")
    .doc(gateway)
    .set(result.dashboard, { merge: true });

  if (gateway === "payu" && result.backfillState) {
    await firestore
      .collection("dashboardMetadata")
      .doc("payuBackfill")
      .set(result.backfillState, {
        merge: true,
      });
  }
}
...
async function main() {
  ...
  const [instamojo, payu, cashfree] = await Promise.all([
    ...
    safeGatewaySync("payu", generatedAt, () =>
      syncPayu(generatedAt, firestore),
    ),
  ...
  const combined = buildDashboardDataFromStoredPayments(
    ...
    combinedDedupe.unique,
    {
      ...
      successfulPaymentCount: combinedDedupe.unique.filter(
        (payment) => payment.status === "Credit",
      ).length,
    },
    generatedAt,
  ...
  const pendingGateways = [
    ...
    payu.dashboard,
    cashfree.dashboard,
  ].filter((gateway) => gateway.syncStatus.state === "pending");
  ...
  if (combinedDedupe.heuristicDuplicateCount > 0) {
    combinedMessageParts.push(
      `${combinedDedupe.heuristicDuplicateCount} heuristic duplicates`,
    );
  }
  ...
  const database = buildCanonicalDatabase(combinedDedupe.unique);
  ...
  if (firestore) {
    try {
      ...
      console.error(
        "Firestore backup skipped after snapshot generation",
        error,
      );
    }
  }
